...
private static final long serialVersionUID = 1L;
  ...
  private DefaultComboBoxModel info= new DefaultComboBoxModel();
  // Code for JCalendar
  ...
  private void jbInit() throws Exception
  {
	...
	for(int i=0; i<rhs.size(); i++){
		((DefaultComboBoxModel) info).addElement(rhs.get(i).toString());
	}
	comboBox = new JComboBox();
	comboBox.setModel(info);
	//comboBox.setModel(new DefaultComboBoxModel(rhs));

	
    this.getContentPane().setLayout(null);
    this.setSize(new Dimension(545, 548));
    this.setTitle("Ofertak kontsultatu");
    jLabel1.setText("Landetxearen kodea:");
    jLabel1.setBounds(new Rectangle(40, 20, 137, 25));
    ...
    scrollPane.setBounds(new Rectangle(45, 305, 452, 116));
    ...
    comboBox.setBounds(189, 22, 226, 20);
 ...
 {		
 		// House object
 		String izena=comboBox.getSelectedItem().toString().split("=>")[0];
 		ApplicationFacadeInterface app;
 		app= MainWindow.getBusinessLogic();
 		RuralHouse rh;
 		try{
 			rh= app.getRuralHouse(izena);
